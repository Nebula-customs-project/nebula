...
export default function ServiceStatusNotification({ status, message }) {
    ...
    switch (status) {
      case "loading":
        return {
          ...
          message:
            message || "Please wait while we connect to the vehicle service...",
        };
      case "error":
        return {
          ...
          message:
            message ||
            "Vehicle service is not started or crashed. Please check the service status.",
        };
      default:
        return null;
    }
  ...
  return (
    ...
    >
      <div
        className={`${config.bgColor} border-2 ${config.textColor} px-6 py-4 rounded-xl shadow-2xl backdrop-blur-md flex items-center gap-4 min-w-[400px] max-w-[600px]`}
      >
...
ServiceStatusNotification.propTypes = {
  status: PropTypes.oneOf(["loading", "error", "connected"]).isRequired,
  message: PropTypes.string,
};

ServiceStatusNotification.defaultProps = {
  message: "",
};
