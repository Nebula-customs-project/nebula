...
const MapView = forwardRef(({
  ...
  useImperativeHandle(ref, () => ({
    ...
    invalidateSize: () => {
      // Delay slightly to allow CSS transitions to complete
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    // Fit map to show the entire route initially
    ...
    setTimeout(() => {
      if (mapRef.current) {
        ...
        // Re-center on current position if following
        if (currentPosition) {
          mapRef.current.setView([currentPosition.lat, currentPosition.lng], 14, { animate: false })
        }
      }
    }, 300)
  ...
  useEffect(() => {
    ...
    if (isFollowing && status === 'IN_PROGRESS') {
  ...
  }, [currentPosition, status, isFollowing, routeCoordinates])
